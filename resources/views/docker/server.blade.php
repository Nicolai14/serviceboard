                    ...
                    <div class="rounded-xl border border-zinc-800 bg-zinc-900 overflow-hidden">
                        ...
                        >
                            <div class="flex items-center gap-3">
                                {{-- Status dot: green if all running, yellow if some, grey if none --}}
                                <span class="h-2 w-2 rounded-full shrink-0"
                                      :class="group.running === group.total
                                          ? 'bg-green-500 shadow-[0_0_5px_theme(colors.green.500)]'
                                          : group.running > 0
                                              ? 'bg-yellow-500 shadow-[0_0_5px_theme(colors.yellow.500)]'
                                              : 'bg-zinc-600'">
                                </span>
                                <span class="text-sm font-semibold text-zinc-100 font-mono" x-text="group.name"></span>
                                <span class="rounded-full border border-zinc-700 bg-zinc-800 px-2 py-0.5 text-xs text-zinc-400">
                        ...
                        <div x-show="isOpen(group.name)" x-collapse>
                            <div class="border-t border-zinc-800/60 divide-y divide-zinc-800/40">
                                <template x-for="c in group.containers" :key="c.container_id">
                                    <div class="px-5 py-4">
                                        <div class="flex flex-wrap items-start justify-between gap-4 mb-3">
                                            ...
                                            <div class="shrink-0 flex flex-col items-end gap-2">
                                                ...
                                                <button @click="toggleNotify(c)"
                                                    ...
                                                    <span x-text="c.notify_on_down ? 'Notify an' : 'Notify aus'"></span>
                                                    {{-- Toggle pill --}}
                                        ...
                                        <template x-if="c.cpu_percent !== null || c.memory_usage_mb !== null">
                                            <div class="grid grid-cols-2 gap-4 pt-3 border-t border-zinc-800/60">
                                                <div>
                                                ...
                                                <div>
                                                    <div class="flex justify-between text-xs mb-1.5">
                                                        ...
                                                        <span :class="c.memory_percent >= 85 ? 'text-red-400' : c.memory_percent >= 60 ? 'text-yellow-400' : 'text-zinc-300'">
                                                            ...
                                                            <template x-if="c.memory_limit_mb">
                                                                <span class="text-zinc-600"> / <span x-text="c.memory_limit_mb + 'MB'"></span></span>
                                                            </template>
                                                        </span>
                                                    </div>
                                                    <div class="h-1.5 w-full rounded-full bg-zinc-800 overflow-hidden">
                                        ...
                                        <template x-if="c.cpu_percent === null && c.memory_usage_mb === null">
                                            <p class="pt-3 border-t border-zinc-800/60 text-xs text-zinc-700">
